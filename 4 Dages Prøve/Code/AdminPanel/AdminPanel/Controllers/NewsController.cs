...
namespace AdminPanel.Controllers
{
    ...
    public class NewsController : Controller
    {
        ...
        {
            var posts = db.Posts.Include(p => p.PostType).Include(p => p.Status).Where(p => p.FK_Type == 1);
        ...
        public ActionResult Create(Posts posts)
        {
            ...
            string stingquery = Request.Form["imagestoadd"];
            if (stingquery != "")
            {
            ...
            string requestCategory = Request.Form["Category"];
            if (requestCategory != null)
            {
                string[] categories = requestCategory.Split(',');
        ...
        [ValidateAntiForgeryToken]
        [ValidateInput(false)]
        public ActionResult CreaQuickNews(GetInformation posts)
        {
            posts.NewsPosts.DateCreated = DateTime.Now;
            posts.NewsPosts.FK_User = (int)Membership.GetUser().ProviderUserKey;
            posts.NewsPosts.FK_Type = 1;
            posts.NewsPosts.FK_Status = 1;
            db.Posts.Add(posts.NewsPosts);
            db.SaveChanges();
